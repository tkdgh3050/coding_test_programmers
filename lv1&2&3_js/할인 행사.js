...
function solution(want, number, discount) {
  ...
  for (let idx = 0; idx < want.length; idx++) {
    totalMap.set(want[idx], number[idx]);
  }

  for (let idx = 0; idx < discount.length; idx++) {
    const now = discount[idx];

    if (totalMap.has(now)) totalMap.set(now, totalMap.get(now) - 1);

    if (queue.length >= 10) {
      const del = queue[qIdx];
      if (totalMap.has(del)) totalMap.set(del, totalMap.get(del) + 1);
      qIdx += 1;
    }

    queue.push(discount[idx]);

    if ([...totalMap.values()].every((val) => val === 0)) answer += 1;
  }
  return answer;
}
